...
act_rows_su <- read_xlsx(file.path(raw_data_folder, year, "Merged", "04_merged_data_substance_use.xlsx")) %>% 
  # Replace all non-breaking spaces (NBSP with regular spaces)
  mutate_all(~ gsub("\u00A0", " ", .))
...
write_xlsx(
  ...
  here("output", year, paste0(year, "_row-validation_substance_use.xlsx"))
)
