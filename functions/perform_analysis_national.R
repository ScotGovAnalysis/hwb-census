perform_analysis_national <- function(hwb_analysis, one_characteristics, stage_and_characteristics, variables) {
  ...
  stage_and_characteristic_list <- lapply(1:nrow(stage_and_characteristics), function(j) {
    ...
    var_result <- lapply(1:nrow(variables), function(i) {
      var_name <- variables$variable[i]
      cat_order <- variables$cat_order[i]
      analysis_stage_and_characteristic(get(dataset), !!as.name(var), !!as.name(var_name), var_name, cat_order)
    }) %>%
      bind_rows()
