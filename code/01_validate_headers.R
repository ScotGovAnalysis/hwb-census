...
header_issues_substance_use <- 
  pmap_dfr(
    expand_grid(all_las),
    ~ check_headers(year, .x, "S4", "01_submitted_data_substance_use", exp_headers$S4_SU, q_pattern)
  )
...
# Structure output as list (each LA will be it's own sheet in excel file)
